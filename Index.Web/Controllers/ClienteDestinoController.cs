...
{
    [IndexAuth]
    public class ClienteDestinoController : Controller
    {
        ...
        {
            IRestResponse WSR = Task.Run(() => apiClient.getJArray("ClienteDestino/Listar", "IdPerson=null")).Result;
        ...
        public ActionResult Create([DataSourceRequest] DataSourceRequest request, Supplier model)
        {
            ...
            if (model != null && ModelState.IsValid)
            {
                ...
                model.IsDestinySupplier = true;
                IRestResponse WSR = Task.Run(() => apiClient.postObject("ClienteDestino/Nuevo", model)).Result;
                if (WSR.StatusCode != HttpStatusCode.OK)
                {
        ...
        public ActionResult Update([DataSourceRequest] DataSourceRequest request, Supplier model)
        {
            ...
            if (model != null && ModelState.IsValid)
            {
                model.RegisterUser = ((Commons.User)Session["USERINFO"]).UserName;
                model.IsDestinySupplier = true;
                IRestResponse WSR = Task.Run(() => apiClient.postObject("ClienteDestino/Modificar", model)).Result;
                if (WSR.StatusCode != HttpStatusCode.OK)
                {
                    ModelState.AddModelError("errorGeneral", JObject.Parse(WSR.Content).ToObject<Error>().Message.ToString());
                }
        ...
        public ActionResult Delete([DataSourceRequest] DataSourceRequest request, Supplier model)
        {
            if (model != null && ModelState.IsValid)
            {
                IRestResponse WSR = Task.Run(() => apiClient.postObject("ClienteDestino/Eliminar", model)).Result;
                if (WSR.StatusCode != HttpStatusCode.OK)
                {
                    ModelState.AddModelError("errorGeneral", JObject.Parse(WSR.Content).ToObject<Error>().Message.ToString());
                }
