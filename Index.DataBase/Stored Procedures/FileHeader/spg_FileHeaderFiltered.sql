CREATE PROCEDURE [spg_FileHeaderFiltered]
	@IdFileHeader		INT,
	@IdCustomer			INT,
	@IdAccount			INT,
...
AS
					...
					IF(ISNULL(@IdAccount, 0) = 0)
						BEGIN
							...
							SELECT	fh.Id, fh.IdCustomer, fh.IdFileInfoConfig, fh.IdDocument, fh.AuthorizationDate, fh.ExpantionDate,
							 ...
							 WHERE	fh.IdDocument like CONCAT('%', @IdDocument, '%')
							   AND	CONVERT(DATE, fh.CreateDate) LIKE CONCAT('%', @CreateDate, '%')
							   AND	CONVERT(DATE, fh.ExpirationDate) LIKE CONCAT('%', @ExpirationDate, '%')
							   AND	CONVERT(DATE, fh.AuthorizationDate) LIKE CONCAT('%', @AuthorizationDate, '%')
						END
					ELSE
						BEGIN
							INSERT INTO @TempFileHeader (Id, IdCustomer, IdFileInfoConfig, IdDocument, AuthorizationDate, ExpantionDate,
	...
	SELECT	fh.Id, fh.IdCustomer, fh.IdFileInfoConfig, fi.DropDownName[FileName], fi.Operation, fi.LoadRawMaterial, fi.IsSubstract,
